...
interface Props {
  open: boolean;
  onClose: () => void;
  onApply: (filters: StoreFilterValues) => void;
  initialFilters: StoreFilterValues;
...
export function StoreFilterModal({
  ...
  const [filters, setFilters] = useState<StoreFilterValues>(initialFilters);
  ...
  const handleReset = () => {
    const emptyFilters: StoreFilterValues = {
      company_id: '',
      status: '',
      country: '',
    };
    ...
    >
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              ...
              >
                {t('stores.filterTitle', 'Filter Stores')}
              </h2>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: '2px 0 0' }}>
                {t('stores.filterSubtitle', 'Refine your store list')}
              </p>
            </div>
          </div>
          <button
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          <div style={{ display: 'grid', gap: '18px' }}>
            ...
            <div>
              ...
              >
                {t('stores.filterStatus', 'Status')}
              </label>
              <CustomSelect
                value={filters.status || null}
                onChange={(value) => setFilters({ ...filters, status: value || '' })}
                options={statusOptions}
                placeholder={t('stores.allStatuses', 'All Statuses')}
                isClearable={true}
                searchable={false}
                highlightSelected={true}
                controlMinHeight={40}
            ...
            {countryOptions.length > 0 && (
              <div>
                ...
                >
                  {t('stores.filterCountry', 'Country')}
                </label>
                <CustomSelect
                  value={filters.country || null}
                  onChange={(value) => setFilters({ ...filters, country: value || '' })}
                  options={countryOptions}
                  placeholder={t('stores.allCountries', 'All Countries')}
                  isClearable={true}
                  searchable={countryOptions.length > 5}
                  highlightSelected={true}
                  controlMinHeight={40}
                />
              </div>
        ...
        >
          ...
          >
            {t('stores.resetFilters', 'Reset All')}
          </button>
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            >
              {t('stores.applyFilters', 'Apply Filters')}
            </button>
          </div>
        </div>
      </div>
